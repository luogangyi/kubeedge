...
type ObjectSyncLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.ObjectSync, err error)
...
// List lists all ObjectSyncs in the indexer.
func (s *objectSyncLister) List(selector labels.Selector) (ret []*v1alpha2.ObjectSync, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.ObjectSync))
	})
	return ret, err
}
...
type ObjectSyncNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.ObjectSync, error)
...
// List lists all ObjectSyncs in the indexer for a given namespace.
func (s objectSyncNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.ObjectSync, err error) {
...
// Get retrieves the ObjectSync from the indexer for a given namespace and name.
func (s objectSyncNamespaceLister) Get(name string) (*v1alpha2.ObjectSync, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("objectsync"), name)
	}
	return obj.(*v1alpha2.ObjectSync), nil
}
